...
export default function NewKoboPage() {
  const { t } = useI18n();
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      setIsLoading(true);
      try {
        const response = await taskService.getTasksByType("new_kobo");
        if (response.data) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
          <FilePlus className="h-6 w-6 text-success" />
          {t("processing.newKoboData")}
        </h1>
